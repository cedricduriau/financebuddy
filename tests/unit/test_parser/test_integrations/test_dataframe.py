...
from financebuddy.parser.integrations.dataframe import pandas_serialize_json
...
def test_read_file(fix_parser_dataframe):
    # CSVParser implements read_file, so it should not raise
    # Just verify it exists and is callable
    assert hasattr(fix_parser_dataframe, "read_file")
    assert callable(fix_parser_dataframe.read_file)


def test_row_to_transaction(fix_parser_dataframe):
...
def test_test_row_to_transaction_fail_no_currency(fix_parser_dataframe):
    row: list[str] = ["1.00"]
...
def test_test_row_to_transaction_no_value(fix_parser_dataframe):
    date = datetime.datetime(1970, 1, 1, tzinfo=datetime.timezone.utc)
